// icons
export { default as AlertIcon } from './icons/icon-alert.svg';
export { default as BetaIcon } from './icons/icon-beta.svg';
export { default as NewIcon } from './icons/icon-new.svg';
export { default as NewBgIcon } from './icons/icon-new-bg.svg';
export { default as MenuIcon } from './icons/icon-menu.png';
export { default as SearchIcon } from './icons/icon-search.svg';
export { default as PrevIcon } from './icons/icon-prev.svg';
export { default as NextIcon } from './icons/icon-next.svg';

// images
export { default as LogoImage } from './images/logo.png';
export { default as ProfileImage } from './images/profile.jpg';
